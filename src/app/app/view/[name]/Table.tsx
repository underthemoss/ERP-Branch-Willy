            ...
            {columns.map((c, i, { length }) => {
              ...
              return (
                <th
                  ...
                  style={{
                    width:
                      i < length - 1 ? TableHeaderRenderer[c].width : "auto",
                  }}
                >
          ...
          <tr>
            <td colSpan={columns.length}>
              <Box gap={1} display={"flex"}>
                ...
                Showing {(data.skip + 1).toLocaleString()} -{" "}
                {(
                  data.skip + Math.min(data.limit, data.results.length)
                ).toLocaleString()}{" "}
                of {data.count.toLocaleString()} results
              </Box>
            </td>
          </tr>
        </tbody>

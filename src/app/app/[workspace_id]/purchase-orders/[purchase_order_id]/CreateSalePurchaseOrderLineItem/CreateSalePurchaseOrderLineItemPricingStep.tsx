...
const CreateSalePurchaseOrderLineItemPricingStep: React.FC<PricingStepProps> = ({
  ...
  const { data, loading, error, refetch } = useGetPurchaseOrderSaleLineItemByIdCreateDialogQuery({
  ...
  React.useEffect(() => {
    if (data?.getPurchaseOrderLineItemById?.__typename === "SalePurchaseOrderLineItem") {

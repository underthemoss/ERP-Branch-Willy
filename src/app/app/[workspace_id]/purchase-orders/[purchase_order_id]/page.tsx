...
export default function PurchaseOrderDisplayPage() {
  ...
  const itemsSection =
    po?.line_items && po.line_items.length > 0 ? (
      <Stack spacing={2}>
        {po.line_items.filter(Boolean).map((item) => {
          if (!item) return null;
          return (
            <Paper key={item.id} elevation={1} sx={{ p: 2 }}>
              <Typography variant="body2">PIM ID: {item.po_pim_id || "-"}</Typography>
              <Typography variant="body2">Quantity: {item.po_quantity ?? "-"}</Typography>
            </Paper>
          );
        })}
      </Stack>

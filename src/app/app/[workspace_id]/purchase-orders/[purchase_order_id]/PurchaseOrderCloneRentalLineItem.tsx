...
import {
  ...
  useGetRentalPurchaseOrderLineItemForCloneQuery,
} from "@/graphql/hooks";
...
graphql(`
  query GetRentalPurchaseOrderLineItemForClone($lineItemId: String!) {
    getPurchaseOrderLineItemById(id: $lineItemId) {
...
export const PurchaseOrderCloneRentalLineItem: React.FC<PurchaseOrderCloneRentalLineItemProps> = ({
  open,
  onClose,
  lineItemId,
  onSuccess,
  ...
  const [createRentalLineItem] = useCreateRentalPurchaseOrderLineItemMutation();

  const lineItem = data?.getPurchaseOrderLineItemById;
  const isRentalItem = lineItem?.__typename === "RentalPurchaseOrderLineItem";
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const input = {
        ...
        po_pim_id: lineItem.po_pim_id,
        po_quantity: lineItem.po_quantity,
...
export default PurchaseOrderCloneRentalLineItem;

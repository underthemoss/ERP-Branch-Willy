...
// Modern Price Card with Add to Quote button
function PriceCard({ hit, onSelect }: { hit: PriceHit; onSelect: (price: PriceHit) => void }) {
...
// Price Grid
function PriceGrid({ onSelect }: { onSelect: (price: PriceHit) => void }) {
  ...
  return (
    ...
    >
      {hits.map((hit) => (
        <PriceCard key={hit.objectID} hit={hit} onSelect={onSelect} />
      ))}
    </div>
  );
}
...
// Props for the browse catalog dialog
export interface BrowseCatalogDialogProps {
  open: boolean;
  onClose: () => void;
  workspaceId: string;
  onSelectPrice: (price: PriceHit) => void;
}

// Main Browse Catalog Dialog Component
            ...
            <div style={{ display: "flex", height: "calc(100% - 80px)" }}>
              ...
              <main style={{ flex: 1, padding: "32px 32px 64px", overflowY: "auto" }}>
                <div style={{ marginBottom: "24px" }}>
                  ...
                  >
                    Select Price for Quote
                  </h1>
                  <p style={{ fontSize: "14px", color: "#6b7280" }}>
                    Search and select a price to add to your quote
                  </p>
                </div>

                <ActiveFilters />
                <ResultsBar />
                <PriceGrid onSelect={handleSelectPrice} />
                <CustomPagination />
              </main>
            </div>
          </InstantSearch>
...
// Export the PriceHit type for use in other components

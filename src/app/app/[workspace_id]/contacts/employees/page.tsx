...
export default function EmployeesContactsPage() {
  ...
  const rows = React.useMemo(() => {
    return (
      data?.listContacts?.items
        ?.filter((contact) => contact?.__typename === "PersonContact")
        .map((contact) => ({
          id: contact?.id ?? "",
          name: contact?.name ?? "",
          phone: contact?.phone ?? "",
      ...
      <Container maxWidth="lg">
        <Box display="flex" alignItems="center" justifyContent="space-between" mt={4} mb={1}>
          ...
          <Button
            ...
            // TODO: Implement create employee contact page/route
        ...
        <Box sx={{ height: 600 }}>
          <div data-testid="employee-list" style={{ height: "100%" }}>
            <DataGridPremium
              columns={columns}
              rows={filteredRows}
              loading={loading}
              ...
              getRowClassName={() => "employee-list-item"}
            />
          </div>
        </Box>
      </Container>

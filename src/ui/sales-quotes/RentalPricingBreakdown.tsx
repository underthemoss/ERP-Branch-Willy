...
export function RentalPricingBreakdown({
  ...
  // Fetch pricing data (skip if no valid dates)
  const { data, loading, error } = useRentalPricingBreakdown_BulkCalculateSubTotalQuery({
    ...
    skip: !startDate || !endDate || durationInDays === 0,
    fetchPolicy: "cache-and-network",
  });

  // Extract total cost (must be before any conditional returns for useEffect)
  const forecast = data?.bulkCalculateSubTotal?.[0];
  const totalCost = forecast?.accumulative_cost_in_cents ?? 0;
  ...
  const savings = lastDay.savings_compared_to_day_rate_in_cents;
  const savingsPercent = lastDay.savings_compared_to_day_rate_in_fraction * 100;

  // Calculate what it would cost with daily rate only
  const totalDays = forecast.days.length;
  const dailyOnlyCost = rates.pricePer1DayInCents * totalDays;
